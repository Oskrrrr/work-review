...
export function ImageStep({ imageId, name, datasetId, onOpen, onReady }: { imageId: string; name?: string; datasetId?: string; onOpen: (url: string, title: string) => void; onReady?: (imageId: string, url: string, title: string) => void }) {
  const [url, setUrl] = useState<string>();
  useEffect(() => {
    let active = true;
    let objectUrl = '';
    loadImage(imageId, datasetId).then(async image => {
      if (!active) return;
