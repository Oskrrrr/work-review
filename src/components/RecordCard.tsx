...
export function RecordCard({ record, onOpenImage, categoryLabel, datasetId, hideContent = false }: { record: WorkRecord; onOpenImage: (url: string, title: string) => void; categoryLabel?: string; datasetId?: string; hideContent?: boolean }) {
  ...
  return <article className="record-card">
    <button className="record-card-head" onClick={() => setExpanded(value => !value)} aria-expanded={expanded}>
      <div><div className="record-meta"><time>{record.date}</time>{!hideContent && record.caseId && <span>{record.caseId}</span>}</div><h3>{hideContent ? '工作内容已隐藏' : record.title}</h3></div>
      <div className="record-actions">{!hideContent && <span className="category-chip">{categoryLabel || record.effectiveCategory}</span>}{!hideContent && imageSteps.length > 0 && <span className="image-count"><Images size={14}/>{imageSteps.length}</span>}{expanded ? <ChevronUp size={17}/> : <ChevronDown size={17}/>}</div>
    </button>
    <p className="record-summary">{hideContent ? '点击右上角“显示内容”后查看工作详情。' : textSteps[0]?.text || (imageSteps.length ? '该事项包含图片处理记录。' : '暂无跟进内容。')}</p>
