...
export function RecordCard({ record, onOpenImage, categoryLabel }: { record: WorkRecord; onOpenImage: (url: string, title: string) => void; categoryLabel?: string }) {
  ...
  return <article className="record-card">
    <button className="record-card-head" onClick={() => setExpanded(value => !value)} aria-expanded={expanded}>
      <div><div className="record-meta"><time>{record.date}</time>{record.caseId && <span>{record.caseId}</span>}</div><h3>{record.title}</h3></div>
      <div className="record-actions"><span className="category-chip">{categoryLabel || record.effectiveCategory}</span>{imageSteps.length > 0 && <span className="image-count"><Images size={14}/>{imageSteps.length}</span>}{expanded ? <ChevronUp size={17}/> : <ChevronDown size={17}/>}</div>
    </button>
    <p className="record-summary">{textSteps[0]?.text || (imageSteps.length ? '该事项包含图片处理记录。' : '暂无跟进内容。')}</p>
    {expanded && <ol className="step-list">{record.steps.map(step => <li key={step.id}>{step.kind === 'text' ? <p>{step.text}</p> : <ImageStep imageId={step.imageId!} name={step.imageName} onReady={registerImage} onOpen={(url, title) => imageSteps.length > 1 ? openGalleryImage(url, title, step.id) : onOpenImage(url, title)}/>}</li>)}</ol>}
