import { ArrowDownWideNarrow, ArrowRight, ArrowUpNarrowWide, CalendarRange, Check, ChevronDown, ChevronUp, Clock3, Eye, EyeOff, Filter, FolderKanban, RotateCcw } from 'lucide-react';
import { useEffect, useMemo, useState } from 'react';
import type { LongTermProject, WorkDataset } from '../types';
import { loadCrossYearProjectsOpen, saveCrossYearProjectsOpen } from '../lib/workFrontPreferences';
...
export function WorkFrontView({ dataset, datasets, projects, onOpenCase, onOpenProject, onChangeLifecycle }: { dataset: WorkDataset; datasets: WorkDataset[]; projects: LongTermProject[]; onOpenCase: () => void; onOpenProject: (projectId: string) => void; onChangeLifecycle: (caseId: string, lifecycle: 'active' | 'completed') => void }) {
  ...
  const [projectIndexOpen, setProjectIndexOpen] = useState(loadCrossYearProjectsOpen);
  useEffect(() => { saveCrossYearProjectsOpen(projectIndexOpen); }, [projectIndexOpen]);
  ...
  return <section className="workspace-view work-front-view">
    ...
    {projectEntries.length > 0 && <article className="panel front-project-index"><div className="panel-header"><div><h2>跨年度长期事项</h2><p>切换年度不会丢失关联项目；点击项目进入完整详情和图片时间线。</p></div><div className="front-project-header-actions"><span>{projectEntries.length} 个主档</span><button type="button" className="front-project-toggle" onClick={() => setProjectIndexOpen(value => !value)} aria-expanded={projectIndexOpen} aria-controls="cross-year-project-list" title={projectIndexOpen ? '收起跨年度长期事项' : '展开跨年度长期事项'}>{projectIndexOpen ? <ChevronUp size={15}/> : <ChevronDown size={15}/>}<span>{projectIndexOpen ? '收起' : '展开'}</span></button></div></div>{projectIndexOpen && <div className="front-project-list" id="cross-year-project-list">{projectEntries.map(entry => { const completedProject = entry.cases.length > 0 && entry.cases.every(item => item.lifecycle === 'completed'); return <div className="front-project-item" key={entry.project.id}><button className="front-project-summary" onClick={() => onOpenProject(entry.project.id)}><span className="front-project-icon"><FolderKanban size={17}/></span><span className="front-project-main"><strong>{entry.project.title}</strong><small>始于 {entry.dates[0]?.slice(0, 4)} 年 · 覆盖 {entry.years.join('、')} · {entry.records.length} 条记录</small></span><span className={`front-project-state${completedProject ? ' completed' : ''}`}>{completedProject ? '已办结' : '进行中'}</span><Eye size={16}/></button></div>; })}</div>}</article>}
