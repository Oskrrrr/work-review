...
export function CasesView({ dataset, projects, onAssignProject, onCreateProject, onChange }: { dataset: WorkDataset; projects: LongTermProject[]; onAssignProject: (caseId: string, projectId?: string) => void; onCreateProject: (caseId: string, title: string) => void; onChange: (dataset: WorkDataset) => void }) {
  ...
  return <section className="workspace-view">
    ...
    {dataset.cases.length ? <div className="case-layout">
      ...
      {selected && <aside className="panel case-detail"><div className="case-detail-head"><div><span className="case-code">{selected.id}</span><h3>{selected.title}</h3></div><button className="unlink-button" onClick={() => detach(selected.id)}><Unlink size={15}/>取消关联</button></div><div className="case-detail-status"><span className={selected.lifecycle === 'completed' ? 'completed' : ''}>{selected.lifecycle === 'completed' ? '已办结，可重新打开' : '进行中'}</span><span>{selected.kind === 'periodic' ? '阶段性工作' : '长期事项'}</span>{selected.longTermProjectId && <span>已关联跨年度主档</span>}</div><p>{selectedRecords[0]?.date} 至 {selectedRecords.at(-1)?.date}</p><div className="lifecycle">{selectedRecords.map(record => <div key={record.id}><time>{record.date}</time><div className="lifecycle-item-head"><strong>{record.title}</strong><button className="icon-button danger lifecycle-delete" onClick={() => detachRecord(record.id)} aria-label={`从 ${selected.id} 中移除 ${record.title}`} title="从此编号中移除"><Trash2 size={14}/></button></div><p>{record.steps.find(step => step.kind === 'text')?.text || '图片处理记录'}</p></div>)}</div></aside>}
    </div> : <div className="large-empty"><FolderKanban size={34}/><h3>还没有已确认的事项编号</h3><p>到“待确认关联”中接受一组建议，或从左侧“自定义关联”手动创建。</p></div>}
    {newProjectFor && <div className="dialog-backdrop project-dialog-backdrop" role="presentation" onMouseDown={event => event.target === event.currentTarget && closeNewProject()}><form className="dialog project-dialog" role="dialog" aria-modal="true" aria-labelledby="new-project-title" onSubmit={submitNewProject}><button type="button" className="dialog-close" onClick={closeNewProject} aria-label="关闭"><X size={19}/></button><div className="dialog-icon"><FolderKanban size={24}/></div><h2 id="new-project-title">新建跨年度长期事项</h2><p>给这一组可以持续多年的工作起一个稳定名称。之后其他年度的同类事项可以选择同一个主档。</p><label className="project-dialog-field">主档名称<input autoFocus value={newProjectTitle} onChange={event => setNewProjectTitle(event.target.value)} placeholder="例如：台山49连建设" maxLength={80}/></label><div className="project-dialog-hint">创建后会自动关联当前事项“{dataset.cases.find(item => item.id === newProjectFor)?.title || newProjectFor}”。</div><div className="project-dialog-actions"><button type="button" className="dialog-secondary" onClick={closeNewProject}>取消</button><button type="submit" className="dialog-primary" disabled={!newProjectTitle.trim()}>创建主档</button></div></form></div>}
  </section>;
}
