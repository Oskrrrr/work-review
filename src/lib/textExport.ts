...
export function buildAiWorklogMarkdown(dataset: WorkDataset) {
  ...
  const confirmedCases = dataset.cases.map(item => ({
    item,
  ...
  if (confirmedCases.length) {
    confirmedCases.forEach(({ item, records: caseRecords }) => {
  ...
  lines.push('## 五、给 AI 的写作提示', '', '请基于以上工作记录，撰写一份正式、客观的年度工作报告：', '- 先概括全年工作主线，再按长期工作项目展开，突出工作的持续周期、关键节点和结果。', '- 将“大类-小类”作为分类线索，但不要机械堆砌分类名称。', '- 对只有过程记录、没有明确结果的事项，使用“完成了相关跟进/推进了相关工作”等谨慎表述，不要虚构成绩。', '- 最后总结工作特点、可复用经验和下一步建议；无法从记录确认的内容请标注“待补充”。');
  return `${lines.join('\n').replace(/\n{3,}/g, '\n\n').trim()}\n`;
}
